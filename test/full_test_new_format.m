...
close all
rng(55);
%% make a test model using [A,B,C,D] = make_test_model(2xCO2EQ,Trans_clim_response,0,Ratio_eq_temp_land_ocean);
[A,B,C,D] = make_test_model(3,3*0.55,0,1.2);
...
%% make a set of NH and SH volcanic optical depth perturbation events
volcano_series = zeros(length(t),2);
volcano_series(170,1) = 0.2; % 2028 NH hemisphere
volcano_series(177,2) = 0.21; % 2035 SH hemisphere
volcano_series(190,1) = 0.18; % 2048 NH hemisphere
volcano_series(192,2) = 0.20; % 2050 SH hemisphere
volcano_series(202,1) = 0.15; % 2060 NH hemisphere

%% make a synthetic industrial aerosol time series
...
for the_year = t(2):2016 % year before control begins
    ...
    % calculate TOA radiation for Norther Hemisphere
    TOA_NH = GHG_forcing(time_ind) ...
        + F_aero(time_ind) ...
        + Fso2 ...
        + volcano_forcing_coef.*NH_v_f ...
        - (SW_out(time_ind) - SW_out(1));
     % calculate TOA radiation for Southern Hemisphere
    TOA_SH = GHG_forcing(time_ind) ...
        + F_aero(time_ind) ...
        + volcano_forcing_coef.*SH_v_f ...
        - (SW_out(time_ind) - SW_out(1));
    U_k = [TOA_NH; TOA_SH];
    % send to simulator to perform one step of the state space model
    [X_k, Y_k] = stateSpaceSim(all_X_states(:,time_ind-1),U_k,dt,A,B,C,D);
    %     T_noise = randn*0;
    %     Y_k = Y_k + T_noise;
    all_X_states(:,time_ind) = X_k;
    all_Y_states(:,time_ind) = Y_k;
    FT = L.*mean(all_Y_states(:,time_ind));
    % calculate net global TOA radiation
    TOA = GHG_forcing(time_ind) ...
        + F_aero(time_ind) ...
        + Fso2./2 ...
        + (volcano_forcing_coef.*NH_v_f + volcano_forcing_coef.*SH_v_f)./2 ...
        -FT ...
        - (SW_out(time_ind) - SW_out(1));
    ...
    % add the 12 items to the spin up data matrix
...
% save the file
% when used with a GCM the GCM expert will have to be able to generate and save this file
% from actual GCM output
save spin_up_data spin_up_data


%% testing script for MPC_for_environmental_management format
% make a test model
...
% quick postscript plot of the main results (stored into <project_name>/figures
IAGP_eazy_plot(pwd, Data, Param, 1980, 2100, F_aero)


% % results plots for my MPC paper comparing various control approaches
